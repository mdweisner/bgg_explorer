...
```{r}
sum(games$no_mech)
mech_miss <- games[games$no_mech == 1, ]
...
colnames(games) <- gsub("  ", " ", colnames(games))
colnames(games) <- gsub(" ", "_", colnames(games))
colnames(games) <- gsub("none", "no_cat", colnames(games))
```
...
cat_miss <- games[games$no_cat == 1, ]
```
Check Both Missing
```{r}
missing_all <- cat_miss[cat_miss$no_mech == 1, ]
```
