...
Drop low vars?
```{r}
sum(games_clean$cat_american_revolutionary_war)
```
...
recommendation <- function(gamename = "Gloomhaven"){
  ...
  head(games_clean$names[which(km_games$cluster == cluster)])
}
```

## Score from 1 to 5
```{r}
recommendation2 <- function(gamename = "Gloomhaven", number = 5){
...
Score[detect, ] <- NA # stops it from recommending itself
set.seed(12345)
  for (i in 1:5){
      km_games <- kmeans(games_numeric_2, centers = 500, nstart = 25)
      cluster <- km_games$cluster[detect]
  ...
  head(arrange(Score, desc(score)), number)
}
```

### Test of Recommendation
```{r error=TRUE}
recommendation2("Betrayal at House on the Hill")
```
...
# rm(cat_miss, game_expansions, games, games2, games_numeric, games_numeric_2, gg, km_games, lm_sub, lm1, mech_miss, missing_all, outliers, pr_games)
#games_clean_training_sub <- head(games_clean_training, n = 120)
#games_clean_testing_sub <- head(games_clean_testing, n = 30)
# rf_grid <- data.frame(.mtry = 2:(ncol(games_clean_training) - 1L)) # across all data
set.seed(12345)
rf_grid <- data.frame(.mtry = 2:30L) # I had to reduce this to get it to run, default is 2/3, maybe change knot size? 
