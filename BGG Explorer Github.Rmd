...
We created a function that uses the linear model to predict the geek rating for any game input, then used that function to add a column of predicted geek ratings to the games_clean dataframe.
### Get Game Prediction
```{r}
prediction <- function(gamename){
  options(warn=-1)
  game_vector <- games_clean_sub[games_clean$names == gamename,]
  return(predict(fit_lm, newdata = game_vector)[[1]])
}
```

### Looking at all predictions
```{r, cache=TRUE, warning=FALSE}
games_with_preds <- games_clean
games_with_preds <- games_with_preds[,c(1, 4, 12, 157, 2, 3, 5:156)]

games_with_preds$predicted_rating <- as.numeric(map(as.character(games_clean$names), prediction))


games_with_preds$names[(games_with_preds$predicted_rating - games_with_preds$geek_rating > 0)]
```
